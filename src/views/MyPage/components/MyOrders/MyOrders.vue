<template>
        ...
        <div class="order" v-for="(item, index) in orderArr">
          <div class="top">
            ...
            <div class="info">
              ...
              <div class="time">
                <p>截止日期：{{ item.time ? item.time : "接单一周内完成" }}</p>
              </div>
              <div class="status">
                <p>
                  状态：
          ...
          <el-dialog
            v-model="isUpload"
            :title="item.status == '完成' ? '查看图片' : '上传图片'"
          >
            <el-upload
              v-model:file-list="orderParams.imgList"
            ...
            >
              <template #tip>
                <div class="el-upload__tip">上传图片（JPG/PNG）</div>
              </template>
              <el-icon>
                <Plus />
              </el-icon>
            ...
            <div class="content-box" v-else>
              <div class="img-box" v-for="preImg in item.imgUrls" :key="preImg">
                <img :src="preImg" alt="" />
              </div>
            </div>
            <template #footer>
              <span class="dialog-footer">
                <el-button
                  ...
                  >退出</el-button
                >
                <el-button @click="confirmUpload(item.id)">确定</el-button>
              </span>
            </template>
          </el-dialog>
        </div>
  ...
  <el-empty v-else description="什么东西都没有~" style="background-color: #fff">
    <a class="toHome" @click="() => $router.push('/appoint')">去逛逛</a>
  </el-empty>
</template>
...
const getResPageList = async () => {
  let res = await reqDesOrderList();
  if (res.code == 200) {
...
const confirmUpload = async (id) => {
  orderParams.value.imgUrls = orderParams.value.imgList.map((item) => {
    return item.response[0];
  });
  orderParams.value.designerId = userStore.userId;
  orderParams.value.orderId = id;

  console.log(orderParams.value);
  let res = await reqUplaodOrderImg(orderParams.value);
  console.log(res);
